...
- Durcissements autopilot (maj 2026-03-23):
  ...
  - si une gate error sorting est active (`FailOnErrorSorting*`), `RunErrorSortingPass` est force automatiquement pour eviter un statut `not_run` ambigu (`error_sorting_pass_forced_by_fail_gate=true`).
  - gates CI decision disponibles: `-FailOnPendingMetricsDecision`, `-FailOnLatestMetricsNotFresh`, `-FailOnNoEffectiveDecision`, `-FailOnEffectiveDecisionNotReadyForBeta`, `-FailOnNonFreshEffectiveDecision`, `-FailOnCachedDecisionSource`, `-FailOnGitDirtyWorktree`, `-FailOnPrismJarSyncNotSynced`, `-FailOnErrorSortingReportMissing`, `-FailOnErrorSortingBlockingPatterns`, `-FailOnErrorSortingStatusNotPass`, `-FailOnErrorSortingNoiseWarn`, `-FailOnErrorSortingNoiseFail`, `-FailOnSummaryOutputWriteError`, `-FailOnMissingSummaryOutput`, `-FailOnSummaryIntegrityMissing` (raison exposee dans `autopilot_failure_reason`, incluant `latest_metrics_not_fresh`, `effective_decision_not_fresh`, `error_sorting_report_missing`, `error_sorting_blocking_patterns`, `error_sorting_status_not_pass`, `error_sorting_noise_fail`, `error_sorting_noise_status_unavailable_for_fail`, `error_sorting_noise_warn_or_worse`, `error_sorting_noise_status_unavailable`, `git_context_unavailable`, `git_dirty_worktree`, `missing_summary_output`, `summary_integrity_missing`; booleen agregat `autopilot_failed`).
  - bundle CI strict disponible: `-EnableStrictCiFailGates` active d'un coup toutes les gates CI strictes et force `RunErrorSortingPass` (etat expose via `strict_ci_fail_gates_enabled`), y compris le rejet des metrics trop anciennes (`latest_metrics_freshness_status`), des decisions non fraiches (`decision_freshness!=fresh`), des worktrees git non propres (`git_is_dirty`), des artefacts error sorting manquants (`error_sorting_reports_present=false`), des statuts error sorting non pass (`error_sorting_status!=pass`) et des seuils noise fail (`error_sorting_known_noise_status=fail`), ainsi que des resumes absents/incomplets (`FailOnMissingSummaryOutput`, `FailOnSummaryIntegrityMissing`); si `SummaryOutputPath` est absent, export auto vers `StrictCiSummaryOutputPath` (defaut `run/pauc_reports/autopilot_summary_ci_strict.json`, trace via `strict_ci_summary_output_defaulted`) et compression JSON forcee par defaut (trace via `strict_ci_summary_output_compress_forced`).
  - export machine-readable disponible: `-SummaryOutputPath <fichier.json>` (`summary_output_compressed`, `summary_output_write_mode`, `summary_output_written/written_utc/size_bytes/sha256/error` dans le resume; mode compact via `-SummaryOutputCompress`), ecriture atomique pour eviter les JSON partiels.
  - coherence resume JSON renforcee: `summary_output_written` + `summary_output_written_utc` sont maintenant renseignes directement dans le fichier ecrit quand la sauvegarde reussit (y compris en run fail-gate).
  - self-test non-regression disponible: `.\tools\test_autopilot_fail_gates.ps1` (baseline + gates decision/qualite `pending_metrics`/`latest_metrics_freshness`/`missing_effective_decision`/`effective_decision_not_fresh`/`cached_decision_source_used`/`effective_decision_not_ready_for_beta`/`prism_jar_sync_not_synced`/`startup_sync_stale_cache_blocked`/`git_context_unavailable`/`git_dirty_worktree` + gates error sorting `blocking/report_missing/status/noise_warn/noise_fail` + verification du bundle strict `EnableStrictCiFailGates` + gates resume `missing_summary_output`/`summary_output_write_error`/`summary_integrity_missing` couvrees en mode echec/succes avec `SummaryOutputPath` valide, plus checks de priorite `summary_output_write_error > missing_summary_output > summary_integrity_missing` et priorite error sorting `error_sorting_report_missing > error_sorting_blocking_patterns > error_sorting_status_not_pass > error_sorting_noise_fail > error_sorting_noise_warn_or_worse` (incluant le cas dedie `noise_status_unavailable_for_fail` prioritaire sur `noise_warn_or_worse` quand `FailOnErrorSortingNoiseFail + FailOnErrorSortingNoiseWarn` sont co-actives), et cas sans `SummaryOutputPath` (meme avec `FailOnSummaryOutputWriteError` actif, cause attendue = `missing_summary_output`), ainsi que scenarios "gate active mais non declenchee" pour error sorting (`FailOnErrorSortingBlockingPatterns`, `FailOnErrorSortingStatusNotPass`, `FailOnErrorSortingNoiseWarn` en status `pass`, `FailOnErrorSortingNoiseFail` en status `warn`) et scenario de declenchement deterministe `FailOnErrorSortingNoiseWarn` en status `warn`, pour `FailOnPrismJarSyncNotSynced` (auto-sync desactive) et pour `FailOnStartupSyncStaleCacheBlock` (pas de cache stale); assertions des flags `fail_on_*`, des inventaires `active_fail_gates`/`triggered_fail_gates` (listes + compteurs) avec attentes exactes `expected_triggered_fail_gates` (y compris cas single-gate error sorting et combinaisons de priorite), checks d'ordre `expected_triggered_fail_gates_in_order`, sous-ensembles obligatoires `required_triggered_fail_gates` (bundle strict) et invariant `autopilot_failure_reason = premiere triggered gate` quand le JSON resume est present, y compris fallback de lecture depuis les logs de cas quand le JSON resume n'est pas present; artefacts de cas path-safe via `case_artifact_token` (index + slug tronque + hash) et garde-fous pre-run (unicite token + limite preventive de longueur de chemin) pour eviter les limites Windows; fallback strict `StrictCiSummaryOutputPath`/`SummaryOutputCompress`, rapport JSON + logs par cas sous `run/pauc_reports/autopilot_fail_gate_selftest_*`).
  - rapport self-test enrichi: export des tableaux `active_fail_gates` et `triggered_fail_gates` dans chaque resultat de cas pour diagnostic direct sans reparser les logs; parsing fallback des logs robuste aux fins de ligne `CRLF`/`CR`/`LF`.
  - invariant cross-check ajoute dans le self-test: chaque `triggered_fail_gate` doit etre justifiee par une gate active compatible via mapping explicite (notamment aliases `git_context_unavailable` et statuts noise error-sorting indisponibles).
  - controles de coherence supplementaires dans le harnais: rejet explicite des doublons `active_fail_gates`/`triggered_fail_gates` et des gates declenchees sans mapping `triggered -> active`.
  - garde-fous pre-run ajoutes: verification de la couverture complete des reasons connues dans le mapping `triggered -> active`, et rejet des noms de cas dupliques.
  - anti-derive script/harnais: extraction automatique des `autopilot_failure_reason` depuis `tools/run_roadmap_autopilot.ps1` et verification qu'elles sont toutes couvertes par les listes/mappings du self-test.
  - determinisme self-test renforce: le scenario `git_context_unavailable` neutralise temporairement `PATH` autour du run, et `git_dirty_worktree` cree puis nettoie automatiquement un probe temporaire (`autopilot_fail_gate_selftest_git_dirty_*.tmp`) pour ne plus dependre de l'etat initial du repo; les scenarios `strict_bundle_*` couvrent explicitement les deux branches git (dirty via probe, context unavailable via `PATH` vide).
  - resume autopilot enrichi: `cached_candidate_is_fresh`, `cached_candidate_eligible_for_use`, `cached_candidate_freshness_status`, `decision_freshness` detaille en cas de `candidate_build_failed`, inventaire des gates CI actives (`active_fail_gate_count`, `active_fail_gates`) et des gates effectivement declenchees (`triggered_fail_gate_count`, `triggered_fail_gates`), plus presence d'artefacts error sorting (`error_sorting_report_md_exists`, `error_sorting_report_json_exists`, `error_sorting_reports_present`, `error_sorting_exit_code`, `error_sorting_error`, `error_sorting_pass_forced_by_fail_gate`) et contexte git (`git_context_available`, `git_branch`, `git_commit`, `git_is_dirty`).
  - coherence `triggered_fail_gates` corrigee: `summary_integrity_missing` n'est plus pre-declenche avant tentative d'ecriture resume; evaluation reservee au bloc post-ecriture.
- Lecture du resultat:
...
- Revalidation outillage beta (2026-03-23):
  ...
  - `-PassThru` renvoie un objet propre (pipeline-safe) pour consommation CI/JSON sans objets de formatage parasites.
  - `-SuppressConsoleSummary` permet de desactiver le resume console (utile en CI quand seul l'objet `-PassThru` est consomme).
  - sortie `-PassThru` enrichie: listes structurees `issues_list` / `warnings_list` en plus des champs string historiques `issues` / `warnings`.
  - validation manifest jar durcie: en multi-jar, `jar_sha256` n'est valide que si `jar_name` pointe un artefact resolvable (evite les faux positifs lies a l'ordre de listing des jars).
  - self-test dedie disponible: `.\tools\test_verify_beta_candidate_passthru.ps1` (verifie absence de fuite d'objets `Format*` et round-trip JSON).
  - self-test comportemental ajoute: `.\tools\test_verify_beta_candidate_behaviors.ps1` (cas deterministes `pass/warn/fail`, mismatch checksum, et verification explicite du throw `-FailOnIssues` sur statuts `warn` et `fail`).
  - self-test contrat integration build->verify ajoute: `.\tools\test_build_beta_candidate_verification_contract.ps1` (verifie les cas `0 sortie`, `sorties multiples`, `sortie sans overall_status`, `sortie valide unique` sur l'appel interne de `build_beta_candidate.ps1`).
  - `tools/build_beta_candidate.ps1` applique desormais un fail-fast sur la sortie du verifier (exactement 1 objet + propriete `overall_status` requise) pour eviter les derives silencieuses de pipeline.
