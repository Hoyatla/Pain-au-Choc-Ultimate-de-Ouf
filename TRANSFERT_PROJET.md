...
- Durcissements autopilot (maj 2026-03-23):
  ...
  - bundle CI strict disponible: `-EnableStrictCiFailGates` active d'un coup toutes les gates CI strictes et force `RunErrorSortingPass` (etat expose via `strict_ci_fail_gates_enabled`); si `SummaryOutputPath` est absent, export auto vers `StrictCiSummaryOutputPath` (defaut `run/pauc_reports/autopilot_summary_ci_strict.json`, trace via `strict_ci_summary_output_defaulted`).
