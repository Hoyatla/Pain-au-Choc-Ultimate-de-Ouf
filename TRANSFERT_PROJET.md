...
- Durcissements autopilot (maj 2026-03-23):
  ...
  - self-test non-regression disponible: `.\tools\test_autopilot_fail_gates.ps1` (baseline + gates error sorting `blocking/status/noise_fail` + verification du bundle strict `EnableStrictCiFailGates`, rapport JSON + logs par cas sous `run/pauc_reports/autopilot_fail_gate_selftest_*`).
