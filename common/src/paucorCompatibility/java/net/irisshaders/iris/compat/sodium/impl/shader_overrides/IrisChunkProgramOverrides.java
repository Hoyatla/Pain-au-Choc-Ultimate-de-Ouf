...
public class IrisChunkProgramOverrides {
	...
	public GlProgram<IrisChunkShaderInterface> getProgramOverride(TerrainRenderPass pass, ChunkVertexType vertexType) {
		...
		boolean shadowPassRequested = ShadowRenderingState.areShadowsCurrentlyBeingRendered();
		boolean useShadowPass = PauCCompatibility.shouldUseSodiumShadowPass();

		if (shadowPassRequested && !useShadowPass) {
			return null;
		}
	...
	public void bindFramebuffer(TerrainRenderPass pass) {
		...
		boolean shadowPassRequested = ShadowRenderingState.areShadowsCurrentlyBeingRendered();
		boolean isShadowPass = PauCCompatibility.shouldUseSodiumShadowPass();

		if (shadowPassRequested && !isShadowPass) {
			return;
		}
