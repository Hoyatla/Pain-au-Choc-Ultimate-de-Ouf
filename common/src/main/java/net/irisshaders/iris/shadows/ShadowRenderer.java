...
public class ShadowRenderer {
	...
	public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
		...
		try {
			...
			ShadowRenderingState.updateShadowTerrainGraphState(cameraX, cameraY, cameraZ, playerCamera.getXRot(), playerCamera.getYRot(),
				renderDistance, intervalSize, getShadowAngle(), halfPlaneLength);
			...
			// Keep vanilla/Sodium terrain setup responsive to shadow camera changes without rebuilding every frame.
			...
			if (levelRenderer instanceof CullingDataCache) {
				((CullingDataCache) levelRenderer).useMainCameraChunksIfShadowSetupFailed();
			}
			...
			ShadowRenderingState.setDisableShadowBlockFaceCulling(this.shouldDisableShadowBlockFaceCulling());
			...
			visibleBlockEntities = null;
			client.smartCull = wasChunkCullingEnabled;
			ShadowRenderingState.setDisableShadowBlockFaceCulling(true);
			levelRenderer.setRenderBuffers(playerBuffers);
			RenderSystem.enableCull();
			Minecraft.getInstance().getMainRenderTarget().bindWrite(false);
			RenderSystem.viewport(0, 0, client.getMainRenderTarget().width, client.getMainRenderTarget().height);

			if (cullingStateSaved && levelRenderer instanceof CullingDataCache) {
				((CullingDataCache) levelRenderer).restoreState();
			}
		}
	}

	private boolean shouldDisableShadowBlockFaceCulling() {
		return this.packHasVoxelization || this.shouldRenderTranslucent || this.packCullingState == ShadowCullState.REVERSED;
	}
