...
public class DispatchingDirectiveHolder implements DirectiveHolder {
	...
	public void processDirective(ConstDirectiveParser.ConstDirective directive) {
		...
		if (type == ConstDirectiveParser.Type.BOOL) {
			...
			if (intConsumer != null) {
				try {
					intConsumer.accept(parseIntegerLiteral(value));
			...
			if (consumer != null) {
				try {
					consumer.accept(parseFloatLiteral(value));
				} catch (NumberFormatException e) {
					Iris.logger.error("Failed to process " + directive, e);
				}
			...
			if (consumer != null) {
				...
				try {
					consumer.accept(new Vector3i(
						parseIntegerLiteral(parts[0]),
						parseIntegerLiteral(parts[1]),
						parseIntegerLiteral(parts[2])
					));
				} catch (NumberFormatException e) {
					Iris.logger.error("Failed to process " + directive, e);
				}
	...
	private static int parseIntegerLiteral(String rawValue) {
		String value = unwrapScalarConstructor(rawValue.trim(), "int");
		if (value.startsWith("float(") && value.endsWith(")")) {
			return (int) parseFloatLiteral(value);
		}
	...
	private static float parseFloatLiteral(String rawValue) {
		String value = unwrapScalarConstructor(rawValue.trim(), "float");
	...
	private static String unwrapScalarConstructor(String value, String constructorName) {
		String prefix = constructorName + "(";
