...
public class ShaderPack {
	...
	public ShaderPack(Path root, Map<String, String> changedConfigs, ImmutableList<StringPair> environmentDefines) throws IOException, IllegalStateException {
		...
		ensureFeatureDeclaredOrAutoEnabled(
			FeatureFlags.SSBO,
			!shaderProperties.getBufferObjects().isEmpty(),
			"SSBOs",
			"SSBO"
		);

		ensureFeatureDeclaredOrAutoEnabled(
			FeatureFlags.CUSTOM_IMAGES,
			!shaderProperties.getIrisCustomImages().isEmpty(),
			"custom images",
			"CUSTOM_IMAGES"
		);
		...
		activeFeatures.stream()
			.filter(flag -> flag != FeatureFlags.UNKNOWN)
			.filter(FeatureFlags::isUsable)
			.map(flag -> "IRIS_FEATURE_" + flag.name())
			.filter(define -> newEnvDefines.stream().noneMatch(pair -> pair.key().equals(define)))
			.forEach(define -> newEnvDefines.add(new StringPair(define, "")));
	...
	private void ensureFeatureDeclaredOrAutoEnabled(FeatureFlags feature, boolean isUsed, String usageDescription, String directiveName) {
		if (!isUsed || activeFeatures.contains(feature)) {
			return;
		}
		...
		if (feature.isUsable()) {
			...
			Iris.logger.warn(
				"Detected {} without {} feature flag declaration. Auto-enabling {} for compatibility; shaderpack should declare \"iris.features.required/optional = {}\".",
				usageDescription, feature.name(), feature.name(), directiveName
			);
			return;
		}

		throw new IllegalStateException(
			"Detected " + usageDescription + ", but " + feature.name()
				+ " is unavailable on this setup and the shaderpack did not declare a compatible fallback. "
				+ "Please set a compatible path with \"iris.features.required/optional = " + directiveName + "\"."
		);
	}
}
