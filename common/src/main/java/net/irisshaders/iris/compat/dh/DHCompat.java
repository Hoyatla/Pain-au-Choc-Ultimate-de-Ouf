...
public class DHCompat {
		...
		try {
			...
			if (isBridgeAvailable()) {
				if (e instanceof ExceptionInInitializerError eiie) {
					...
					throw new RuntimeException("DH shader bridge is available, but one or more API methods are missing. PauC requires the embedded DH API bridge to match Iris' DH compatibility layer.", e);
				}
			} else {
	...
	public static boolean isUsingPauCEmbeddedBridgeOnly() {
		return PauCEmbeddedDhRuntime.shouldExposeToShaderBridge()
			&& !IrisPlatformHelpers.getInstance().isModLoaded("distanthorizons");
	}
	...
	public static int getShaderRenderDistance() {
		int realRenderDistance = getRenderDistance();
		if (!isUsingPauCEmbeddedBridgeOnly()) {
			return realRenderDistance;
		}

		return PauCLodShaderPresentation.shaderRenderDistanceBlocks(realRenderDistance);
	}
