...
public class LodRendererEvents {
	...
	private static boolean irisDhEventsBound = false;

	private static boolean atTranslucent = false;
	private static int textureWidth;
	private static int textureHeight;
	private static boolean degradedShaderLodLogged;
	private static boolean dhFogOverrideApplied;
	private static boolean dhAmbientOcclusionOverrideApplied;
	...
	private static void bindIrisDhEvents() {
		if (irisDhEventsBound) {
			return;
		}

		irisDhEventsBound = true;
	...
	private static void setupSetDeferredBeforeRenderingEvent() {
		DhApiBeforeRenderEvent beforeRenderEvent = new DhApiBeforeRenderEvent() {
			...
			public void beforeRender(DhApiCancelableEventParam<DhApiRenderParam> event) {
				if (shouldPauseDegradedShaderLod()) {
					logDegradedShaderLodPauseOnce();
	...
	private static void setupGenericEvent() {
		...
		DhApiBeforeGenericObjectRenderEvent beforeDrawEvent = new DhApiBeforeGenericObjectRenderEvent() {
			...
			public void beforeRender(DhApiCancelableEventParam<EventParam> dhApiCancelableEventParam) {
				DHCompatInternal compat = getInstance();
				if (dhApiCancelableEventParam.value.resourceLocationPath.equalsIgnoreCase("Clouds")) {
					if (compat.shouldOverride && compat.avoidRenderingClouds()) {
	...
	private static void setupTransparentRendererEventCancling() {
		DhApiBeforeRenderEvent beforeRenderEvent = new DhApiBeforeRenderEvent() {
			@Override
			public void beforeRender(DhApiCancelableEventParam<DhApiRenderParam> event) {
				if (ShadowRenderingState.areShadowsCurrentlyBeingRendered() && !shouldRenderNativeDhShadow(getInstance())) {
					event.cancelEvent();
				}
			}
		};
		DhApiBeforeDeferredRenderEvent beforeRenderEvent2 = new DhApiBeforeDeferredRenderEvent() {
			...
			public void beforeRender(DhApiCancelableEventParam<DhApiRenderParam> event) {
				if (PauCLodShaderSafety.shouldSuppressShaderTransparentLodPass()) {
					logShaderTransparentSuppressionOnce();
					event.cancelEvent();
					return;
				}
			...
			public void beforeCleanup(DhApiEventParam<DhApiRenderParam> event) {
				...
				if (instance.shouldOverride) {
					if (ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
						if (shouldRenderNativeDhShadow(instance)) {
							instance.getShadowShader().unbind();
						}
					} else if (atTranslucent && instance.hasTranslucentOverride()) {
						instance.getTranslucentShader().unbind();
					} else {
		...
		DhApiBeforeRenderSetupEvent beforeRenderPassEvent = new DhApiBeforeRenderSetupEvent() {
			...
			public void beforeSetup(DhApiEventParam<DhApiRenderParam> event) {
				...
				if (instance.shouldOverride) {
					...
					} else if (!ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
		...
		DhApiBeforeRenderPassEvent beforeCleanupEvent = new DhApiBeforeRenderPassEvent() {
			...
			public void beforeRender(DhApiEventParam<DhApiRenderParam> event) {
				...
				if (event.value.renderPass == EDhApiRenderPass.TRANSPARENT) {
					...
					if (instance.shouldOverride && !instance.hasTranslucentOverride()) {
						atTranslucent = false;
						logShaderTransparentUnavailableOnce();
						return;
					}
	...
	private static void applyPauCDhRuntimeOverrides() {
		if (DhApi.Delayed.configs == null) {
			return;
		}

		DhApi.Delayed.configs.graphics().ambientOcclusion().enabled().setValue(false);
		dhAmbientOcclusionOverrideApplied = true;
		DhApi.Delayed.configs.graphics().fog().drawMode().setValue(EDhApiFogDrawMode.FOG_DISABLED);
		dhFogOverrideApplied = true;
		if (setDhCoreConfigApiValue(DH_QUALITY_CONFIG_CLASS, DH_FAR_CLIP_FADE_FIELD, Boolean.FALSE)) {
			dhFarClipFadeOverrideApplied = true;
			if (!dhFarClipFadeOverrideLogged) {
				dhFarClipFadeOverrideLogged = true;
				Iris.logger.info("PauC disabled DH far-clip fade while shader LOD override is active.");
			}
		}
	}

	private static void clearPauCDhRuntimeOverrides() {
		if (DhApi.Delayed.renderProxy != null) {
			DhApi.Delayed.renderProxy.setDeferTransparentRendering(false);
		}

		if (DhApi.Delayed.configs == null) {
			dhFogOverrideApplied = false;
			dhAmbientOcclusionOverrideApplied = false;
			dhFarClipFadeOverrideApplied = false;
			return;
		}

		if (dhFogOverrideApplied) {
			DhApi.Delayed.configs.graphics().fog().drawMode().clearValue();
			dhFogOverrideApplied = false;
		}

		if (dhAmbientOcclusionOverrideApplied) {
			DhApi.Delayed.configs.graphics().ambientOcclusion().enabled().clearValue();
			dhAmbientOcclusionOverrideApplied = false;
		}

		if (dhFarClipFadeOverrideApplied) {
	...
	private static boolean setDhCoreConfigApiValue(String configClassName, String fieldName, Object value) {
		try {
			...
			if (!dhFarClipFadeOverrideMissingLogged) {
				dhFarClipFadeOverrideMissingLogged = true;
				Iris.logger.warn("PauC could not override DH far-clip fade; DH version may expose a different config path.", throwable);
			}
			return false;
		}
	}

	private static boolean shouldPauseDegradedShaderLod() {
		String explicit = System.getProperty("pauc.dh.pauseDegradedShaderLod");
		if (explicit != null) {
		...
		if (PauCEmbeddedDhRuntime.shouldExposeToShaderBridge() && !IrisPlatformHelpers.getInstance().isModLoaded("distanthorizons")) {
			return false;
		}

		IrisPlatformHelpers helpers = IrisPlatformHelpers.getInstance();
	...
	private static boolean shouldRenderNativeDhShadow(DHCompatInternal instance) {
		return instance != null
			...
			&& PauCLodShaderRuntime.shouldRenderNativeDhShadowThisFrame()
			&& instance.getShadowShader() != null
			&& instance.getShadowFB() != null
			&& instance.getShadowFBWrapper() != null;
	}

	private static void logDegradedShaderLodPauseOnce() {
		if (!degradedShaderLodLogged) {
			degradedShaderLodLogged = true;
			Iris.logger.warn("PauC paused Distant Horizons LOD rendering while shaders are active because the Forge terrain bridge stack is degraded; vanilla shader terrain remains active.");
		}
	}

	private static void logShaderTransparentSuppressionOnce() {
		if (!shaderTransparentSuppressionLogged) {
			shaderTransparentSuppressionLogged = true;
			Iris.logger.info("PauC suppressed shader LOD transparent pass to avoid unstable water/reflection compositing.");
		}
	}

	private static void logShaderTransparentUnavailableOnce() {
