	...
	public static void addFogUniforms(DynamicUniformHolder uniforms, FogMode fogMode) {
		if (fogMode == FogMode.OFF) {
			...
			uniforms.uniform1i("fogMode", () -> {
				float fogDensity = PauCLodShaderPresentation.shaderFogDensity(CapturedRenderingState.INSTANCE.getFogDensity());
		...
		uniforms.uniform1f("fogDensity", () -> {
			...
			return Math.max(0.0F, PauCLodShaderPresentation.shaderFogDensity(CapturedRenderingState.INSTANCE.getFogDensity()));
		}, notifier -> {
		});

		uniforms.uniform1f("fogStart", () -> PauCLodShaderPresentation.shaderFogStartBlocks(RenderSystem.getShaderFogStart()), listener -> StateUpdateNotifiers.fogStartNotifier.setListener(listener));

		uniforms.uniform1f("fogEnd", () -> PauCLodShaderPresentation.shaderFogEndBlocks(RenderSystem.getShaderFogEnd()), listener -> StateUpdateNotifiers.fogEndNotifier.setListener(listener));
