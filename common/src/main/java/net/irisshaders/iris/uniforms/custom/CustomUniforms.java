...
public class CustomUniforms implements FunctionContext {
	...
	private CustomUniforms(CustomUniformFixedInputUniformsHolder inputHolder, Map<String, Builder.Variable> variables) {
		...
		ExpressionResolver resolver = new ExpressionResolver(
			...
			(name) -> {
				...
				Type shaderConstantType = CustomUniformShaderConstants.getType(name);
				if (shaderConstantType != null)
...

		{
			...
			for (Map.Entry<String, Expression> entry : this.variablesExpressions.entrySet()) {
				...
				for (VariableExpression v : requires) {
					...
					try {
						evaluated = v.partialEval(this, functionReturn);
					} catch (RuntimeException e) {
						dependencyFailed = true;
						brokenUniforms.add(uniform);
						Iris.logger.warn(
							"Failed to resolve dependency {} for custom uniform {}, disabling it.",
							v,
							uniform.getName(),
							e
						);
					...
					} else if (!(evaluated instanceof ConstantExpression)) {
						// Any other expression here means we failed to fold a dependency into a usable value.
						brokenUniforms.add(uniform);
					} else {
						// Constants do not create runtime dependencies.
					}
				}
	...
	private void addVariable(Expression expression, CachedUniform uniform) throws Exception {
		...
		if (CustomUniformShaderConstants.getType(name) != null)
			throw new Exception("Variable shadows built in shader constant: " + name);
	...
	public boolean hasVariable(String name) {
		return this.inputHolder.containsKey(name) || this.variables.containsKey(name) || CustomUniformShaderConstants.getType(name) != null;
	}
	...
	public Expression getVariable(String name) {
		...
		Expression shaderConstant = CustomUniformShaderConstants.getExpression(name);
