...
public class StandardMacros {
	...
	private static final int[] LEGACY_IRIS_WORD = new int[] {73, 82, 73, 83}; // "IRIS"
	...
	public static ImmutableList<StringPair> createStandardEnvironmentDefines() {
		...
		String paucVersion = normalizeVersionLiteral(getFormattedIrisVersion());
		String irisCompatVersion = getIrisCompatibilityVersion();

		define(standardDefines, "MC_VERSION", getMcVersion());
		define(standardDefines, "IRIS_VERSION", irisCompatVersion);
		define(standardDefines, "PAUC_VERSION", paucVersion);
		...
		// Preserve OptiFine/Iris shaderpack compatibility even when branded jars sanitize legacy names.
		define(standardDefines, legacyMacro("IS_", null));
		define(standardDefines, legacyMacro(null, "_VERSION"), irisCompatVersion);
		define(standardDefines, legacyMacro(null, "_TAG_SUPPORT"), "2");
		define(standardDefines, "PAUC_LOD");


		if (DHCompat.isBridgeAvailable() && (DHCompat.hasRenderingEnabled() || DHCompat.isUsingPauCEmbeddedBridgeOnly())) {
			define(standardDefines, "DISTANT_HORIZONS");
			define(standardDefines, "PAUC_LOD_DEPTH");
		}

		if (DHCompat.isUsingPauCEmbeddedBridgeOnly()) {
			define(standardDefines, "PAUC_EMBEDDED_DH");
			if (PauCLodShaderPresentation.isPresentationEnabled()) {
				define(standardDefines, "PAUC_LOD_SHADER_PRESENTATION");
			}
		}

		if (IrisPlatformHelpers.getInstance().isModLoaded("continuity")) {
			define(standardDefines, "IRIS_HAS_CONNECTED_TEXTURES");
			define(standardDefines, legacyMacro(null, "_HAS_CONNECTED_TEXTURES"));
		}
		...
		define(standardDefines, "PAUC_LOD_BLOCK_UNKNOWN", String.valueOf(0));
		define(standardDefines, "PAUC_LOD_BLOCK_LEAVES", String.valueOf(1));
		define(standardDefines, "PAUC_LOD_BLOCK_STONE", String.valueOf(2));
		define(standardDefines, "PAUC_LOD_BLOCK_WOOD", String.valueOf(3));
		define(standardDefines, "PAUC_LOD_BLOCK_METAL", String.valueOf(4));
		define(standardDefines, "PAUC_LOD_BLOCK_DIRT", String.valueOf(5));
		define(standardDefines, "PAUC_LOD_BLOCK_LAVA", String.valueOf(6));
		define(standardDefines, "PAUC_LOD_BLOCK_DEEPSLATE", String.valueOf(7));
		define(standardDefines, "PAUC_LOD_BLOCK_SNOW", String.valueOf(8));
		define(standardDefines, "PAUC_LOD_BLOCK_SAND", String.valueOf(9));
		define(standardDefines, "PAUC_LOD_BLOCK_TERRACOTTA", String.valueOf(10));
		define(standardDefines, "PAUC_LOD_BLOCK_NETHER_STONE", String.valueOf(11));
		define(standardDefines, "PAUC_LOD_BLOCK_WATER", String.valueOf(12));
		define(standardDefines, "PAUC_LOD_BLOCK_GRASS", String.valueOf(13));
		define(standardDefines, "PAUC_LOD_BLOCK_AIR", String.valueOf(14));
		define(standardDefines, "PAUC_LOD_BLOCK_ILLUMINATED", String.valueOf(15));
	...
	private static String legacyMacro(@Nullable String prefix, @Nullable String suffix) {
		StringBuilder builder = new StringBuilder(32);
		if (prefix != null) {
			builder.append(prefix);
		}
		for (int codePoint : LEGACY_IRIS_WORD) {
	...
	private static String getIrisCompatibilityVersion() {
		String override = System.getProperty("pauc.shader.irisCompatVersion");
		...
		if (trimmed.chars().allMatch(Character::isDigit)) {
			return normalizeVersionLiteral(trimmed);
		}

		String formatted = formatVersionString(trimmed);
		if (formatted != null) {
			return normalizeVersionLiteral(formatted);
		}

		Iris.logger.warn("Invalid pauc.shader.irisCompatVersion '{}', falling back to 10706.", override);
		return "10706";
	}

	private static String normalizeVersionLiteral(String value) {
