...
public class LayoutTransformer {
	...
	public static void transformGrouped(
		...
		for (ShaderType type : pipeline) {
			...
			Root.indexBuildSession(currentRoot, () -> {
				if (lastMap[0] != null) {
					transformIn(lastMap[0], t, currentTree, currentRoot, parameters);
				}

				lastMap[0] = transformOut(t, currentTree, currentRoot, parameters);
			});
