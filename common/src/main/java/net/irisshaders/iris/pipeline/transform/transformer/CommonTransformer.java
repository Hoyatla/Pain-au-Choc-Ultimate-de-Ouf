...
public class CommonTransformer {
	...
	public static final AutoHintedMatcher<Expression> glTextureMatrix2 = new AutoHintedMatcher<>(
		"gl_TextureMatrix[2]", Matcher.expressionPattern);
	...
	static void renameAndWrapShadow(ASTParser t, Root root, String oldName, String innerName) {
		root.process(root.identifierIndex.getStream(oldName)
			...
			id -> {
				...
				FunctionCallExpression wrapper = (FunctionCallExpression) t.parseExpression(functionCall, "vec4()");
	...
	public static void transform(
		...
		if (parameters.type.glShaderType == ShaderType.VERTEX) {
			...
			tree.prependMain(t, "iris_FogFragCoord = 0.0f;");
	...
	public static boolean hasExternalDeclaration(Root root, String name) {
		return root.nodeIndex.getStream(DeclarationExternalDeclaration.class)
			.map(DeclarationExternalDeclaration::getDeclaration)
			.filter(TypeAndInitDeclaration.class::isInstance)
			.map(TypeAndInitDeclaration.class::cast)
			.flatMap(declaration -> declaration.getMembers().stream())
