...
public class PipelineManager {
	...
	public WorldRenderingPipeline preparePipeline(NamespacedId currentDimension) {
		if (PauCRenderLifecycle.isClientLogoutInProgress()) {
			// Avoid expensive shader pipeline construction while the client is shutting down.
			return pipeline != null ? pipeline : new VanillaRenderingPipeline();
		}
	...
	public boolean hasActivePipelines() {
		return !pipelinesPerDimension.isEmpty();
	}

	public int getActivePipelineCount() {
		return pipelinesPerDimension.size();
	}
	...
	public void destroyPipelineForClientLogout() {
		PauCRenderLifecycle.beginClientLogoutPipelineDestroy();
		try {
			destroyPipeline();
		} finally {
			PauCRenderLifecycle.endClientLogoutPipelineDestroy();
		}
	}
