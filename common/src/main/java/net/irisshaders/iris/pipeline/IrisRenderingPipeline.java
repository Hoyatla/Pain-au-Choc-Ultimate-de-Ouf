...
public class IrisRenderingPipeline implements WorldRenderingPipeline, ShaderRenderingPipeline, RenderTargetStateListener {
	...
	private boolean reportedTerrainOverrideWithoutMainTarget;
	...
	public IrisRenderingPipeline(ProgramSet programSet) {
		...
		WorldRenderingSettings.INSTANCE.setUseExtendedVertexFormat(this.sodiumTerrainPipeline.requiresExtendedVertexFormat());
	...
	public void destroy() {
		...
		if (!PauCRenderLifecycle.isClientLogoutPipelineDestroyActive()) {
	...
	public boolean shouldOverrideShaders() {
		...
		boolean terrainPhase = switch (getPhase()) {
			case TERRAIN_SOLID, TERRAIN_CUTOUT_MIPPED, TERRAIN_CUTOUT, TERRAIN_TRANSLUCENT, TRIPWIRE -> true;
			default -> false;
		};

		if (terrainPhase && !reportedTerrainOverrideWithoutMainTarget) {
			reportedTerrainOverrideWithoutMainTarget = true;
			Iris.logger.info("PauC keeps terrain shader override active while a non-main framebuffer is bound.");
		}
	...
	@Override
	public boolean supportsSodiumShadowPass() {
		return shadowRenderer != null && sodiumTerrainPipeline != null && sodiumTerrainPipeline.hasShadowPass();
	}
