...
public class DHTerrainTransformer {
	public static void transform(
		...
		root.rename("dhModelView", "iris_ModelViewMatrix");
		root.rename("dhModelViewInverse", "iris_ModelViewMatrixInverse");
		...
		root.replaceReferenceExpressions(t, "gl_NormalMatrix",
			"iris_NormalMatrix");
		addIfNotExists(root, t, tree, "iris_NormalMatrix", Type.F32MAT3X3, StorageQualifier.StorageType.UNIFORM);

		addIfNotExists(root, t, tree, "iris_ModelViewMatrixInverse", Type.F32MAT4X4, StorageQualifier.StorageType.UNIFORM);

		addIfNotExists(root, t, tree, "iris_ProjectionMatrixInverse", Type.F32MAT4X4, StorageQualifier.StorageType.UNIFORM);

		if (parameters.type.glShaderType == ShaderType.FRAGMENT) {
			addDhDepthSamplerIfReferenced(t, tree, root, "dhDepthTex");
			addDhDepthSamplerIfReferenced(t, tree, root, "dhDepthTex0");
			addDhDepthSamplerIfReferenced(t, tree, root, "dhDepthTex1");
		}

		Iris.logger.debug("Transforming DH terrain shader type " + parameters.type);
		...
		if (parameters.type.glShaderType == ShaderType.VERTEX) {
			...
			addIfNotExists(root, t, tree, "iris_ProjectionMatrix", Type.F32MAT4X4, StorageQualifier.StorageType.UNIFORM);
			addIfNotExists(root, t, tree, "iris_ModelViewMatrix", Type.F32MAT4X4, StorageQualifier.StorageType.UNIFORM);
	...
	private static void addDhDepthSamplerIfReferenced(ASTParser t, TranslationUnit tree, Root root, String name) {
		if (root.identifierIndex.has(name) && !CommonTransformer.hasExternalDeclaration(root, name)) {
			tree.parseAndInjectNode(t, ASTInjectionPoint.BEFORE_DECLARATIONS, "uniform sampler2D " + name + ";");
		}
	}
