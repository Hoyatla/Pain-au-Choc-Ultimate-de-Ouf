...
public class RenderTargets {
	...
	private DepthBufferFormat copiedDepthFormat;
	...
	public RenderTargets(int width, int height, int depthTexture, int depthBufferVersion, DepthBufferFormat depthFormat, Map<Integer, PackRenderTargetDirectives.RenderTargetSettings> renderTargets, PackDirectives packDirectives) {
		...
		this.copiedDepthFormat = depthFormat.toDepthOnlyFormat();
		...
		this.noTranslucents = new DepthTexture("depthtex1", width, height, copiedDepthFormat);
		this.noHand = new DepthTexture("dephtex2", width, height, copiedDepthFormat);
	...
	public boolean resizeIfNeeded(int newDepthBufferVersion, int newDepthTextureId, int newWidth, int newHeight, DepthBufferFormat newDepthFormat, PackDirectives packDirectives) {
		...
		if (depthFormatChanged) {
			...
			copiedDepthFormat = newDepthFormat.toDepthOnlyFormat();
		...
		if (depthFormatChanged || sizeChanged) {
			...
			noTranslucents.resize(newWidth, newHeight, copiedDepthFormat);
			noHand.resize(newWidth, newHeight, copiedDepthFormat);
			this.translucentDepthDirty = true;
			this.handDepthDirty = true;
		}
	...
	public void copyPreTranslucentDepth() {
		if (translucentDepthDirty) {
			...
			IrisRenderSystem.copyTexImage2D(GL20C.GL_TEXTURE_2D, 0, copiedDepthFormat.getGlInternalFormat(), 0, 0, cachedWidth, cachedHeight, 0);
		} else {
			depthSourceFb.bindAsReadBuffer();
			IrisRenderSystem.copyTexSubImage2D(noTranslucents.getTextureId(), GL20C.GL_TEXTURE_2D, 0, 0, 0, 0, 0,
	...
	public void copyPreHandDepth() {
		if (handDepthDirty) {
			...
			depthSourceFb.bindAsReadBuffer();
			IrisRenderSystem.copyTexSubImage2D(noHand.getTextureId(), GL20C.GL_TEXTURE_2D, 0, 0, 0, 0, 0,
				getCurrentWidth(), getCurrentHeight());
		}
	}
