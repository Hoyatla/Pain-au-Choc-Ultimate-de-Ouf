...
uniform int paucVanillaFogStartDistance;
uniform int paucVanillaFogEndDistance;
uniform int paucPhotonShadowRequestedDistance;
...
float pauc_photon_transition_fog(float view_distance) {
    float requested_distance = float(max(paucPhotonShadowRequestedDistance, 0));
    float effective_distance = float(max(paucPhotonShadowEffectiveDistance, 0));
    float transition_start = max(float(paucVanillaFogStartDistance), 32.0);
    float transition_end =
        max(float(paucVanillaFogEndDistance), transition_start + 32.0);

    if (effective_distance > 0.0) {
        transition_start = max(32.0, min(transition_start, effective_distance));
    } else if (requested_distance > 0.0) {
        transition_start = max(32.0, min(transition_start, requested_distance));
    }

    if (effective_distance > 0.0) {
        transition_end = max(transition_end, effective_distance + 16.0);
    }
    if (requested_distance > 0.0) {
        transition_end = max(transition_end, requested_distance + 16.0);
    }

    return 1.0 - smoothstep(transition_start, transition_end, view_distance);
}
...
#else
    float view_distance = length(scene_pos.xz);
    float horizon_fog = view_distance / float(lod_render_distance);
    float fog = exp2(-2.4 * sqr(horizon_fog));
    if (paucPhotonShadowRequestedDistance > 0 ||
        paucPhotonShadowEffectiveDistance > 0 ||
        paucVanillaFogEndDistance > paucVanillaFogStartDistance) {
        fog = min(fog, pauc_photon_transition_fog(view_distance));
    }
#endif
