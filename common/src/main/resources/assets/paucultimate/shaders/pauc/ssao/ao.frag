...
float GetSpiralOcclusion(const in vec2 uv, const in vec3 viewPos, const in vec3 viewNormal) {
    ...
    for (int i = 0; i < clamp(uSampleCount, 1, SAMPLE_MAX); i++) {
        vec2 offset = vec2(sin(rotatePhase), cos(rotatePhase)) * radius;
        radius += rStep;
...
void main() {
    ...
    if (fragmentDepth < 1.0) {
        ...
        float distFromCam = length(viewPos);
        if (distFromCam < uFadeDistanceInBlocks) {
            ...
            occlusion *= (uFadeDistanceInBlocks - distFromCam) / uFadeDistanceInBlocks;
        }
    }
